...
const Company = ({ data: company, dbRef }: { data: CompanyType, dbRef: 'Company' | 'Client' }) => {
  const { name } = company;
  return <AccordionItem border={'1px solid gray'} m="2" borderRadius="10px">
    <AccordionButton>
      <Text>{name}</Text>
    </AccordionButton>
    <AccordionPanel pb="4">
...
const CompanyPanel = ({ company, dbRef }: { company: CompanyType, dbRef: 'Company' | 'Client' }) => {
  ...
  return showUpdateForm ? <UpdatePanel company={company} onClose={() => setShowUpdateForm(false)} dbRef={dbRef} /> :
    <DisplayPanel company={company} onClose={() => setShowUpdateForm(true)} dbRef={dbRef} />
}

const UpdatePanel = ({ company, dbRef, onClose }: { company: CompanyType, onClose: () => void, dbRef: 'Company' | 'Client' }) => {
...
const DisplayPanel = ({ company, onClose, dbRef }: { company: CompanyType, onClose: () => void, dbRef: 'Company' | 'Client' }) => {
  const { name, id, cui, line1, line2, line3, line4, line5 } = company;
  const { t } = useTranslation();
  return <Stack alignItems='flex-start' spacing={2}>
    <Box>
      <Text><b>{t('Name')}</b>: {name}</Text>
    </Box>
    <Box>
      <Text><b>{t('Reg No')}</b>: {cui}</Text>
    </Box>
    <Stack alignItems='flex-start' alignContent='flex-start' spacing={1}>
      <Text fontWeight='bold'>{t('Invoice data: ')}</Text>
      <Text fontSize='14px' textAlign='start'>{line1}</Text>
      <Text fontSize='14px' textAlign='start'>{line2}</Text>
      <Text fontSize='14px' textAlign='start'>{line3}</Text>
      <Text fontSize='14px' textAlign='start'>{line4}</Text>
      <Text fontSize='14px' textAlign='start'>{line5}</Text>
    </Stack>
