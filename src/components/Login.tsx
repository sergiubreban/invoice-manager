...
const Login = ({ login }: { login: () => void }) => {
  ...
  return <form onSubmit={handleSubmit(onSubmit)}>
    <VStack spacing={4}>
      <FormControl>
        <FormLabel htmlFor='username' {...(errors.username && { color: 'red' })}>username</FormLabel>
        <Input id='username' placeholder='username'  {...register("username", { required: true })} />
      </FormControl>
      <FormControl>
        <FormLabel htmlFor='password'  {...(errors.password && { color: 'red' })}>password</FormLabel>
      ...
      <Flex width='100%' justify='space-between' direction='row-reverse'>
        ...
        {credentialsErr && <Text color='red' fontSize={'12px'}>wrong credentials</Text>}
      </Flex>
    </VStack>
  </form>
}
